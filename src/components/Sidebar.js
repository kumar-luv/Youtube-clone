...
export default function Sidebar() {
  ...
  return ( toggleMenu &&
    <div className="w-2/12 pb-8 pr-5 overflow-auto sticky top-0 z-30 h-full dark:text-white">
      ...
      <ul className="flex flex-col border-b-2  border-gray-700">
        {secondaryLinks.map(({ icon, name }) => {
          return (
            <li key={name} className={`pl-6 py-3 hover:bg-zinc-600 flex items-center gap-5 hover:cursor-pointer`}>
                {icon}
                <span className="text-sm tracking-wider">{name}</span>{" "}
            </li>
          );
